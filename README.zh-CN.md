...
先确认 Docker Desktop 或其他兼容的 Docker daemon 已经启动。默认会使用带 Python 环境的 Debian 容器，因此宿主机本身不需要预装 `dpkg-deb`。
...
可以先用下面的命令初始化一套专用签名密钥，并导出可安装的公钥 keyring：
...
`--no-protection` 适合本地暂存与验证；如果后面要长期公开发布，建议改用 `--passphrase`。

然后对暂存仓库签名，并把公钥文件导出到 `apt-repo/public`：
...
如果要托管到 GitHub Pages，请配置：

- 仓库变量：`APT_GPG_KEY_ID`
- 仓库密钥：`APT_GPG_PRIVATE_KEY`
- 可选仓库密钥：`APT_GPG_PASSPHRASE`

然后把 GitHub Pages 的 Source 设为 `GitHub Actions`，再运行 `Publish APT Repository` 工作流。
